...
for(let i = 0; i < Math.floor(Math.random() * 5) + 3; i++) {
    let mass = Math.floor(Math.random() * 20) + 5; // Random mass between 5 and 25
 ...
 */
function draw() {
    /* Only update if background is on */
...
/**
 * backgroundCheck - Enable the background?
 * Updates with the checkbox
 */
function backgroundCheck() {
    let checkbox = document.getElementById('bg-checkbox');

    canvas.style.display = checkbox.checked ? 'inline': 'none';
    document.cookie = 'background=' + (checkbox.checked) + '; path=/';
    console.log(document.cookie)
}

if (document.cookie.startsWith('background=false')) {
    canvas.style.display = 'none';
    document.getElementById('bg-checkbox').checked = false;
}
